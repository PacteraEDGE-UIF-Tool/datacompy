...
from PyQt6.QtCore import QDir, Qt, QItemSelectionModel, QItemSelection, QThread, pyqtSignal
from PyQt6 import QtCore
from PyQt6 import QtGui,uic
import glob
from PyQt6.QtWidgets import QTableWidgetItem
from PyQt6.QtWidgets import QMessageBox , QProgressBar,QDialog
from data_cleaner import cleanfile
from prefixspan_pattern_generator import split_data_main
from SettingWindowAdapter import CSettingWindowAdapter
import collections
import configparser
import ast
import time
...
class CMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def actionBtnCleanDataClicked(self):
        if self.original_file=='' :
             ...
             return
        self.ProgressBarDialog=CProgressBarDialog(self.original_file)
    ...
    def actionBtnCompPairClicked(self):
        ...
        if self.SelectedFile1=='' or self.SelectedFile2=='':
            QMessageBox.information(None,'Error', 'Please selct the the file you want to compare first.', QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Close)
            return
        ...
        if not self.config.has_option("DATA_INFO", 'columns_names'):
            QMessageBox.information(
                None,
                'Error',
                "The columns name config doesn't exist, config value is generated by data clean.",
                 QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Close
            )
            return
        column_names= ast.literal_eval(self.config.get("DATA_INFO",'columns_names'))
        print(column_names)
        ...
        self.ResultUI= CResultWindowAdapter(
            ...
            table1_column_names=column_names,
            table2_column_names=column_names,
    ...
    def actionBtnSelectOrigFileClicked(self):
        self.original_file=QFileDialog.getOpenFileName(
            self, 'Open file', 
            self.current_path,"All files (*.*)"
            )[0]
        self.label.setText(self.original_file)
    ...
    def actionSettingTriggered(self):
        self.SettingWindow = QtWidgets.QWidget()
        self.SettingUI= CSettingWindowAdapter()
        self.SettingUI.setupUi(self.SettingWindow)
        self.SettingUI.setupData()
    ...
    def setupUIFunctionalities(self):
        ...
        #TO DO !!!!!!!!!!!!!!!!!!!
        self.DataLimitCount=99999999999999999
        ...
        self.lineEdit_2.setValidator(QIntValidator(1, 9999999, self))
        ...
        self.actionsetting.triggered.connect(self.actionSettingTriggered)

    def __init__(self):
        super().__init__()
        
        self.setupUi(self)
        self.setWindowIcon(QtGui.QIcon("./img/diff.png"))
        self.setWindowTitle("File comparing tool")
